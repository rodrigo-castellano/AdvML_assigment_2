...
class Tree:
        ...
        while cur_num_nodes < num_nodes:
            ...
            if num_children > 0:
                ...
                for i in range(num_children):
                    print('children ',i)
                    cat = []
                    for theta in range(k): #for every children there's k=2 values, i.e., (x1,x2) and each value has k=2 prob
                        cat.append(np.random.dirichlet(alpha))
                    print('cat', cat)
                    child_node = Node(str(cur_num_nodes), cat)
                    child_node.ancestor = cur_node
    ...
    def sample_tree(self, num_samples=1, seed_val=None):
        ...
        print('ini samples filt',samples,filtered_samples)
        if self.num_nodes > 0:
            ...
            for sample_idx in range(num_samples):
                ...
                while len(visit_list) != 0:
                    ...
                    print('curr node',cur_node.name, 'visit',len(visit_list), 'descendants', len(cur_node.descendants))
                    par_node = cur_node.ancestor

                    if cur_node == self.root:
                        cat = cur_node.cat  # the cat for the first node is an option with k probs
                    else:
                        # for the other nodes, there are k options. The chosen option is, for ex., if the parents has choices nr. 0,1 and 
                        # the chosen is 0, then the current node will go for option 0.
                        par_k = int(samples[sample_idx, int(par_node.name)]) 
                        print('par_k', par_k,'ancestor', par_node.name , 'sampples',samples)
                        cat = cur_node.cat[par_k]
                    cur_sample = np.random.choice(np.arange(self.k), p=cat)
                    print('cur sample', cur_sample, 'between ', np.arange(self.k), 'with prob ',cat)
                    print('---------------------------------')
...
def main():
    ...
    # t2.sample_tree(num_samples=5)
    # # t.sample_tree(num_samples=1)
    # t2.print()
    
    # t3 = Tree()
    # t3.load_tree("data/q2_2_small_tree.pkl")#(filename)
    # t3.print()
